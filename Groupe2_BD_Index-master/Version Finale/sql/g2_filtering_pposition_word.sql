...
CREATE PROCEDURE FILTERING_PPOSITION_WORD (IN v_position INT, IN v_word VARCHAR(25), IN v_lemma VARCHAR(25),IN v_title BOOLEAN, IN v_pos_tag VARCHAR(25), IN v_type_entity VARCHAR(25), IN v_id_article VARCHAR(50))
	...
	BEGIN
		...
		IF (v_id_lemma IS NULL AND LENGTH(v_lemma) > 2)  THEN
		...
		IF (v_id_entity IS NULL AND LENGTH(v_type_entity) > 2)  THEN
			INSERT INTO entity (id_entity, type_entity) VALUES (NULL, v_type_entity);
			SELECT LAST_INSERT_ID() INTO v_id_entity;
		END IF;

		IF (v_id_pos_tag IS NULL AND LENGTH(v_pos_tag) > 2)  THEN
